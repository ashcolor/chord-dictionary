...
export default {
  ...
  methods: {
    dispScore: function() {
      ...
      var octave = CLEFS[this.settings.clef].octave + (this.chord.original[0].key < CLEFS[this.settings.clef].note);
      const notes = new VF.StaveNote({
        clef: this.settings.clef,
        keys: this.chord.original.map(function(item, index) {
          return "CDEFGAB".charAt(item.key) + "/" + (this.chord.original[index - 1] && item.key <= this.chord.original[index - 1].key ? ++octave : octave);
        }.bind(this)),
      ...
      this.chord.original.forEach((note, index) => {
        ...
        if (this.settings.isColorNote) notes.setKeyStyle(index, { fillStyle: COLORS[note.toHalf()] });
      });

      const voice = new VF.Voice({ num_beats: 1, beat_value: NOTES[this.settings.note].num });
      ...
      renderer.resize(box.w + 80, Math.max(Math.abs(box.y) + box.h + 15 + (this.settings.note != "whole") * 15, 110));
