...
export default {
  ...
  methods: {
    playChord: function() {
      ...
      this.insts[this.settings.inst].triggerAttackRelease(this.chordVoicing.map(midi => Tone.Frequency(midi, "midi")), 1.5);
      /*
      const part = new Tone.Part(
        function setPlay(time, note) {
          this.insts[this.settings.inst].releaseAll();
          this.insts[this.settings.inst].triggerAttackRelease(note, 1.5, time);
        }.bind(this),
